...
const checkAttempts = ({...props}) => {
  if (
    attempts === 6 ||
    (currentNameGuess === props.chosenFlag.name &&
      currentYearGuess === props.chosenFlag.year)
  ) {
    ...
    elem.innerHTML = `${currentNameGuess}${currentYearGuess}`
    elem.id = 'nm'
    guessBox.insertBefore(elem, guessBox.firstChild)
    const remainingGuess = 6 - attempts
    for (let i = remainingGuess; i === 0; i--) {
...
const onNIncorrect = (guessName, ...props) => {
  console.log('nah')
}
    ...
    <Box id="namebox">
      <FormControl required fullWidth>
        ...
        >
          ...
          <MenuItem value="Twink">Twink</MenuItem>
        </Select>
      </FormControl>
    </Box>
  )
